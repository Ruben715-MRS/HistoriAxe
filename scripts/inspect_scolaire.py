...
scolaires = [(p, ev) for p, ev in events if 'Programmes scolaires' in p and len(ev.get('titre', '').split()) in (7, 8)]
print(f"Programmes scolaires: {len(scolaires)} long events")
for p, ev in scolaires[:30]:
    t = ev['titre']
    print(f"[{len(t.split())}w] ({ev['id']}) {t}")
