...
# Superfluous adjectives / adverbs
REDUNDANT_WORDS = [
    r"\bmajestueux\b",
    r"\bmajestueuse\b",
    r"\bdouloureux\b",
...
def shorten_one(title, path=""):
    ...
    # 1. Apply phrase substitutions
    for pat, rep in PHRASE_SUBS:
        t = re.sub(pat, rep, t)
    t = clean_spaces(t)
    if len(t.split()) <= 6:
        return t

    # 2. Check if removing redundant words helps
    for rw in REDUNDANT_WORDS:
        cand = re.sub(r'\s+' + rw, '', t)
        cand = re.sub(rw + r'\s+', '', cand)
        cand = clean_spaces(cand)
        if len(cand.split()) < len(t.split()):
            t = cand
            if len(t.split()) <= 6:
                return t

    # 3. Check leading articles
    w = t.split()
    ...
    # 4. Check comma clauses
    if ',' in t and len(t.split()) > 6:
        parts = [p.strip() for p in t.split(',')]
        if 3 <= len(parts[0].split()) <= 6:
            # If second part is location or qualifier
            p2 = parts[1].split()
            if len(p2) <= 3:
                return parts[0]
