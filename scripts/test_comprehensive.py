...
# Specific dictionary for known titles
KNOWN_TITLES = {
...
PHRASES = [
    ...
    # Locutions adverbiales et prépositionnelles
...
def process_title(ev_id, title, path=""):
    if ev_id in KNOWN_TITLES:
        return KNOWN_TITLES[ev_id]
    ...
    # 2. Qualifiers pruning
    for q in QUALIFIERS:
        cand = re.sub(r'\s+' + q, '', t)
        cand = re.sub(q + r'\s+', '', cand)
        cand = clean(cand)
    ...
    # 6. Trailing location trimming (e.g., " à Paris", " à Londres", " à Rome", " à New York")
...
for p, ev in long_events:
    ...
    if len(res.split()) <= 6:
        ...
        still_long.append((p, ev.get('id', ''), ev['titre'], res))
