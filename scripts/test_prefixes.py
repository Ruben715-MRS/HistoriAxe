...
for prefix in ['Naissance', 'Mort', 'Publication', 'Fondation', 'Création', 'Premier', 'Première']:
    sub = [(p, ev) for p, ev in long_events if ev['titre'].split()[0] == prefix]
    print(f"=== Prefix {prefix} ({len(sub)} events) ===")
    for p, ev in sub[:6]:
        print(f"  {ev['titre']}")
